...
| PDF 文本提取 | `pdftotext` | 尝试 Agent 自带 PDF 能力；否则请求 DOCX、TXT 或复制文本 |
| PDF 页面渲染 | `pdftoppm` | 继续文本审计；明确说明未完成表图视觉核对；请求截图 |
...
4. 对 Markdown 原文进行 HTML escaping；
5. 保留 Unicode；
6. 不依赖 `pretty-doc`；
...
| 场景 | 用户体验 |
|---|---|
| 缺少 `pdftoppm` | 继续文本审计；报告注明“未视觉检查表图”；请求表图截图 |
| 缺少 `pdftotext` | 尝试 Agent 内置读取；否则请求 DOCX、TXT 或复制文本 |
...
3. 执行用户代码前，先说明将运行哪些文件、读取哪些路径、写入哪些路径，并获得用户确认。
4. 默认避免将原始数据复制到报告目录。
5. 报告中避免暴露参与者身份信息、邮箱、手机号、组织内部路径或未脱敏数据片段。
6. 对原始数据只做完成任务所需的最小读取。
7. 不把第三方 API 或联网服务设为默认路径。
