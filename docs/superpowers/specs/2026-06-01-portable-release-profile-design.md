...
4. 对 Markdown 原文进行 HTML escaping，只激活 `http://`、`https://`、`#`、单斜杠开头的本地绝对路径、`./` 和 `../` 链接；不激活 `//` 开头的远程主机形式，也不激活包含反斜杠或编码分隔符的本地路径；其他链接目标作为无交互文本显示。编码分隔符限制只针对本地路径，不误伤正常 HTTPS URL。
5. 保留 Unicode；
6. 不依赖 `pretty-doc`；
7. 在可用时尝试打开浏览器，但打开失败不影响交付。
8. 默认无覆盖写入必须使用原子方式创建文件；显式覆盖必须原子替换目录条目。两种模式都不能跟随符号链接目标。
...
6. HTML 渲染器正确 escape 不可信文本，不激活非允许列表链接，并以原子方式避免默认覆盖或跟随符号链接目标。
